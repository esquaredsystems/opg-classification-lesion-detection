...
TRAIN_DIR_NAME = 'train_lesion_detection'
TEST_DIR_NAME = 'test_lesion_detection'
HEIGHT = 832
WIDTH = 832
...
class DentalOPGConfig(Config):
    ...
    NUM_CLASSES = 1 + 1  # background + 1 (lesion type)
...
class OPGDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
        ...
        try:
            ...
            print(f'\nValueError: {ex} when loading masks for image: {image_id}\n')
            print("; ".join(self.class_info))
...
def generate_teeth_annotations(data, save_in=TRAIN_DIR_NAME, save_annotated_images=True, resize=False):
    '''
    Reads all rows from data and generates a text file from all lesion annotations.
    ...
    sample = data[1]
    teeth = json.load(open(DATA_DIRECTORY + sample['lesion-annotations-file'], 'r'))
    ...
    for shape in teeth['shapes']:
        ...
        if class_name and class_name != '1':
            class_name = '1'
    ...
    annotation_json = {
        'info': {'contributor': "Owais Hussain", 'date_created': "2023-01-25", 'description': "Dental Lesion annotations"},
        'images': [], 'annotations': []}

    for row in data:
        annotations_file = row['lesion-annotations-file']
        print("Generating lesion annotations for: " + annotations_file)
        ...
        if len(teeth['shapes']) == 0:
            print(f'Skipping {annotations_file} because it is empty')
            continue
        ...
        raw_filename = 'raw-lesion-' + annotations_file.replace('.json', '') + '-' + str(
        ...
        if save_annotated_images:
            annotated_filename = save_in + 'bounded-lesion-' + annotations_file.replace('.json', '') + '-' + str(
                encrypt_name(annotations_file.replace('.json', ''))) + '.png'
    ...
    # output
    with open(save_in + 'lesion-annotations.json', 'w') as f:
...
# To split the dataset into training and test and generate annotations.json files from teeth annotations
split_into_test_train = False
# If pre-trained model already exist, then use them (keep True in production)
retrain_model = False
...
data = [x for x in data if x['year'] == '2022'
        and x['source'] in ['aku', 'riphah', 'tufts', 'ufb']
        and x['lesion-annotations-file'] is not None]

# Remove troublesome data (invalid images, empty annotations, etc.)
invalid_ids = ['lsn-rph22-96.jpg', 'lsn-tft22-750.jpg', 'lsn-tft22-503.jpg']
data = [x for x in data if x['lesion-annotations-file'] not in invalid_ids]
...
dataset_val.load_data(WORKING_DIRECTORY + TEST_DIR_NAME + '/lesion-annotations.json', WORKING_DIRECTORY + TEST_DIR_NAME)
dataset_val.prepare()
dataset_train = OPGDataset()
dataset_train.load_data(WORKING_DIRECTORY + TRAIN_DIR_NAME + '/lesion-annotations.json', WORKING_DIRECTORY + TRAIN_DIR_NAME)
...
if retrain_model:
    ...
    aug_options = []
    # Crop or pad each side by 25% relative to original size (negative values are for cropping, positive for padding)
    aug_options.append(aug.CropAndPad(percent=(-0.25, 0.25)))
    # Convert each image to grayscale and alpha-blend to remove about 50% of all color
    aug_options.append(aug.BlendAlpha(0.5, aug.Grayscale(1.0)))
    # Flip 50% of all images horizontally
    aug_options.append(aug.Fliplr(0.5))
    # Flip 50% of all images vertically
    aug_options.append(aug.Flipud(0.5))
    # Scale images to a value of 75 to 125% of their original size
    aug_options.append(aug.Affine(scale=(0.75, 1.25)))
    # Rotate images by -30 to 30 degrees
    aug_options.append(aug.Affine(rotate=(-30, 30)))
    # Sheer the images by -15 to 15 degrees
    aug_options.append(aug.Affine(shear=(-15, 15)))

    model.train(dataset_train, dataset_val, learning_rate=config.LEARNING_RATE, epochs=500,
                augmentation=aug.Sometimes(0.5, aug_options), layers='heads')
    end_train = time.time()
...
# If the above doesn't work, try: model_path = os.path.join("lesion_logs")
...
#         r = results[0]
#         plt = visualize.display_instances(
#             img, r['rois'], r['masks'], r['class_ids'], dataset_val.class_names, r['scores'])
#         plt.savefig(f"{real_test_dir}/lesion_results/lesion-{image_file}", dpi=300)
#     except:
#         pass
...
df.to_csv(real_test_dir + 'lesion_results/lesion_results.csv')


# Check the segment_results directory, and try to run predictions on all segmented images
# segment_results_file = f"{real_test_dir}/segment_results/segment_results.csv"
# if os.path.exists(segment_results_file):
#     # Read the resulst CSV into data frame
#     df = pd.read_csv(segment_results_file)
#     # For each row, search for the image and predict lesion
#     for idx, row in df.iterrows():
#         image_path = f"{real_test_dir}/segment_results/{row['class_id']}-{row['image']}"
#         try:
#             img = skimage.io.imread(image_path)
#             img_arr = np.array(img)
#             results = model.detect([img_arr], verbose=1)
...
#                 plt = visualize.display_instances(
#                     img, r['rois'], r['masks'], r['class_ids'], dataset_val.class_names, r['scores'])
#                 plt.savefig(f"{real_test_dir}/lesion_results/lesion-{image_path}", dpi=300)
#         except:
#             pass
