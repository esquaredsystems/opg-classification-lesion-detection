...
class Config:
    def __init__(self):
        ...
        # Original anchor_box_scales in the paper is [128, 256, 512]. Set to [64, 128, 256] to save training time
        self.anchor_box_scales = [128, 256, 512]
        # Anchor box ratios
        self.anchor_box_ratios = [[1, 1], [1. / math.sqrt(2), 2. / math.sqrt(2)],
                                  [2. / math.sqrt(2), 1. / math.sqrt(2)]]
        # Size to resize the smallest side of the image. Original is 600. Set to 300 to save training time
        self.im_size = 600
...
def classifier_layer(base_layers, input_rois, num_rois, nb_classes=4, pooling_regions=7):
    ...
    out = TimeDistributed(Dropout(0.5))(out)
    # softmax acivation function for classify the class name of the object
...
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
    ...
    # RPN has many more negative than positive regions, so we turn off some of the negative regions
    num_regions = 256  # We limit regions to 256
...
def augment(img_data, config, augment_images=True):
    ...
    while True:
        for img_data in all_img_data:
            try:
                ...
                if mode == 'train':
                    img_data_aug, x_img = augment(img_data, C, augment_images=True)
                else:
                    img_data_aug, x_img = augment(img_data, C, augment_images=False)
...
def non_max_suppression_fast(boxes, probs, overlap_thresh=0.9, max_boxes=600):
    # Code: http://www.pyimagesearch.com/2015/02/16/faster-non-maximum-suppression-python/
    # Process explanation:
...
def apply_regr_np(X, T):
    ...
    try:
        ...
        x1 = cx1 - w1 / 2.
        y1 = cy1 - h1 / 2.
        x1, y1, x2, y2 = np.round(x1), np.round(y1), np.round(w1), np.round(h1)
...
train_path = WORKING_DIR + 'annotation_train.txt'  # Training data (annotation file)
config_output_filename = os.path.join(WORKING_DIR + 'model_vgg_config.pickle')
...
C.record_path = os.path.join(WORKING_DIR + 'record.csv')
C.model_path = os.path.join(WORKING_DIR + 'model_frcnn_vgg.hdf5')
C.num_rois = 4
C.base_net_weights = os.path.join(WORKING_DIR + 'vgg16_weights_tf_dim_ordering_tf_kernels.h5')
...
# e.g. classes_count: {'Car': 2383, 'Mobile phone': 1108, 'Person': 3745, 'bg': 0}
# e.g. class_mapping: {'Person': 0, 'Car': 1, 'Mobile phone': 2, 'bg': 3}
...
print("Analysis on first image in training set...")
print(f"Original image: height={image_data['height']}; width={image_data['width']}")
print(f"Resized image: height={X.shape[1]}; width={X.shape[2]}; C.im_size={C.im_size}")
print(f"Feature map size: height={Y[0].shape[1]}; width={Y[0].shape[1]} C.rpn_stride={C.rpn_stride}")
print(X.shape)
print(str(len(Y)) + " includes 'y_rpn_cls' and 'y_rpn_regr'")
print(f'Shape of y_rpn_cls {Y[0].shape}')
print(f'Shape of y_rpn_regr {Y[1].shape}')
print(image_data)

print(f"Number of positive anchors for this image: {debug_num_pos}")
if debug_num_pos == 0:
